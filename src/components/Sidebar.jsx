  ...
  return (
    <div>
      ...
      >
        ...
        <span className="text-[10px] text-stone-500 ml-1">{open ? '▾' : '▸'}</span>
      </button>
      {open && (
    ...
    <aside className="w-52 min-w-[208px] h-screen flex flex-col bg-brand-dark border-r border-white/[0.06] overflow-y-auto">
      ...
      <div className="px-5 py-5 border-b border-white/[0.06]">
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 rounded-lg bg-brand-gold/20 border border-brand-gold/30 flex items-center justify-center flex-shrink-0">
            <span className="text-brand-gold font-display font-bold text-sm">W</span>
          </div>
          <div>
            <div className="text-white font-display font-bold text-sm leading-tight tracking-wide">WRANGL</div>
            <div className="text-stone-500 text-[10px] tracking-wider uppercase">Berkely Distribution</div>
          </div>
        </div>
      </div>
        ...
        {NAV.map(group => (
          <div key={group.section} className="mb-2">
            ...
            {group.items.map(item =>
              item.collapsible ? (
                <CollapsibleItem key={item.label} item={item} location={location} />
              ) : (
                <NavLink
                  key={item.to}
                  to={item.to}
      ...
      <div className="px-4 py-4 border-t border-white/[0.06]">
        ...
        <button
          ...
          className="w-full text-left text-stone-400 hover:text-stone-200 text-xs py-1.5 px-2 rounded-lg hover:bg-white/5 transition-all duration-150 mb-1 flex items-center gap-2"
        >
          <span>💬</span> Send Feedback
        </button>
        <button
          onClick={handleSignOut}
          className="w-full text-left text-stone-500 hover:text-stone-300 text-xs py-1.5 px-2 rounded-lg hover:bg-white/5 transition-all duration-150"
        >
          Sign out
        </button>
      </div>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6">
            ...
            {sent ? (
              ...
              <>
                <textarea
                  className="w-full border border-stone-200 rounded-xl p-3 text-sm text-stone-700 focus:outline-none focus:ring-2 focus:ring-brand-gold/30 focus:border-brand-gold/50 resize-none h-32"
                ...
                <div className="flex gap-3 mt-4">
                  <button onClick={() => setShowFeedback(false)} className="flex-1 py-2 px-4 rounded-xl border border-stone-200 text-sm text-stone-500 hover:bg-stone-50 transition-colors">
                    Cancel
                  </button>
                  <button
